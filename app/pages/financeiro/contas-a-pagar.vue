<template>
  <div style="padding: 24px;">
    ...
    >
      ...
      <n-button type="primary" @click="openCreateModal">Nova Conta</n-button>
    </div>
    ...
    <n-grid x-gap="12" :cols="4" style="margin-bottom: 24px;">
        ...
        <n-card title="Total Geral do Mês">
          <n-statistic>
            <span style="font-size: 24px; font-weight: bold;">{{
              formatCurrency(dashboardData?.kpis?.expectedTotal || 0)
            }}</span>
          </n-statistic>
        </n-card>
      </n-grid-item>
      <n-grid-item>
        <n-card title="Total Pendente">
          <n-statistic>
            <span style="font-size: 24px; font-weight: bold; color: #f0a020;">{{
              formatCurrency(dashboardData?.kpis?.totalPending || 0)
            }}</span>
          </n-statistic>
        </n-card>
      </n-grid-item>
      <n-grid-item>
        <n-card title="Total Pago">
          <n-statistic>
            <span style="font-size: 24px; font-weight: bold; color: #18a058;">{{
              formatCurrency(dashboardData?.kpis?.totalPaid || 0)
            }}</span>
          </n-statistic>
        </n-card>
      </n-grid-item>
      <n-grid-item>
        <n-card title="Atrasado">
          <n-statistic>
            <span style="font-size: 24px; font-weight: bold; color: #d03050;">{{
              formatCurrency(dashboardData?.kpis?.totalOverdue || 0)
            }}</span>
          </n-statistic>
        </n-card>
      </n-grid-item>
      ...
      <n-form :model="createForm" ref="createFormRef" :rules="createRules">
        ...
        <n-form-item label="Categoria" path="category">
          <n-select
            ...
            :options="categoryOptions.filter((o) => o.value !== 'Todas as Despesas')"
          />
        </n-form-item>
...
const rowClassName = (row: AccountPayableItem) => {
...
const columns = [
  ...
  },
  {
    ...
    render(row: AccountPayableItem) {
      if (row.status === 'PAID') {
        return h(NTag, { type: 'success' }, { default: () => 'PAGO' });
      }
      ...
      if (due.valueOf() < today.valueOf()) {
        return h(NTag, { type: 'error' }, { default: () => 'ATRASADO' });
      }
...
const fetchPayables = async () => {
  ...
  try {
    ...
    if (filters.category !== 'Todas as Despesas') {
...
const handleUndo = async (row: AccountPayableItem) => {
