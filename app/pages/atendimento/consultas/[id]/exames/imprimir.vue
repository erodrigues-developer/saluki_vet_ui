  ...
  <div class="prescription-print-page">
    ...
    <n-spin :show="loading">
      <div v-if="loadError" class="state-card">
        <h2>Não foi possível carregar o pedido de exames</h2>
        <p>Verifique se existe pedido vinculado à consulta #{{ consultationId }}.</p>
      </div>
      ...
      <article v-else-if="documentReady" class="page">
        ...
        <section class="document-head">
          <div class="meta-grid">
            <p><strong>Pedido nº:</strong> {{ examRequestCode }}</p>
            <p><strong>Atendimento nº:</strong> {{ attendanceCode }}</p>
            <p><strong>Data de emissão:</strong> {{ formatDateTime(issuedAt) }}</p>
        ...
        <section class="doc-section">
          ...
          <ul class="exam-list">
            <li v-for="item in requestedExams" :key="item.id">
              {{ item.name }}<span v-if="item.category"> · {{ item.category }}</span>
            </li>
          </ul>
        </section>

        <section v-if="notesText" class="doc-section">
          <h3>Observações</h3>
          <p>{{ notesText }}</p>
        </section>
        ...
        <section class="signature-section">
          ...
          <p class="vet-title">{{ veterinarianName }}</p>
          <p>Solicitante</p>
        </section>

        <footer class="document-footer">
          <p>
            Documento gerado em {{ generationDate }} · Pedido {{ examRequestCode }} · {{ clinic.name }}
          </p>
        </footer>
      </article>
    </n-spin>
...
const veterinarianRaw = reactive<any>({})
const clinicSettings = reactive<any>({})
const examRequests = ref<any[]>([])
...
const orFallback = (value: unknown) => safeText(value) || fallbackLabel
...
const issuedAt = computed(() => examRequests.value[0]?.requestedAt || consultation.updatedAt || consultation.createdAt || new Date().toISOString())
const examRequestCode = computed(() => {
  const id = Number(examRequests.value[0]?.id || 0)
  if (!id) return 'EX-N/I'
  const year = new Date(issuedAt.value || Date.now()).getFullYear()
  return `EX-${year}-${String(id).padStart(6, '0')}`
})
...
const requestedExams = computed(() =>
  examRequests.value.map((item) => ({
    id: Number(item.id),
    name: orFallback(item.examType?.name),
    category: safeText(item.examType?.examCategory?.name),
  })),
)
const notesText = computed(() => safeText(examRequests.value[0]?.notes))
const returnRecommended = computed(() => {
  const notes = safeText(consultation.notes)
  const match = notes.match(/Retorno\s*:\s*(.+)/i)
  return match?.[1]?.trim() || ''
})
const veterinarianName = computed(() => orFallback(veterinarianRaw.name))
const generationDate = computed(() => formatDateTime(generationTimestamp.value))
const documentReady = computed(() => Number(consultation.id) > 0 && requestedExams.value.length > 0)
...
const fetchData = async () => {
  ...
  try {
    const [consultationRes, examRequestsRes, clinicRes] = await Promise.all([
      api<any>(`/api/v1/consultations/${consultationId}`),
      api<any>('/api/v1/exam-requests', { query: { consultationId } }),
      api<any>('/api/v1/clinic-settings')
    ])

    Object.assign(consultation, consultationRes || {})
    Object.assign(clinicSettings, clinicRes || {})
    examRequests.value = Array.isArray(examRequestsRes?.data) ? examRequestsRes.data : []

    if (!examRequests.value.length) {
      loadError.value = true
      return
    }

    const [petRes, tutorRes, veterinarianRes] = await Promise.all([
      api<any>(`/api/v1/pets/${consultation.petId}`),
      api<any>(`/api/v1/clients/${consultation.clientId}`),
      api<any>(`/api/v1/users/${consultation.veterinarianId}`),
    ])

    Object.assign(pet, petRes || {})
    Object.assign(tutorRaw, tutorRes || {})
    Object.assign(veterinarianRaw, veterinarianRes || {})
